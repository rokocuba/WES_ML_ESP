...
bool allocate_tensor_arena(size_t arena_size)
{
    ...
    s_tensor_arena = static_cast<uint8_t *>(
        heap_caps_aligned_alloc(16, arena_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)
    );
    if (s_tensor_arena != nullptr) {
        s_tensor_arena_size = arena_size;
        ESP_LOGI(kTag, "Tensor arena allocated in PSRAM (%u bytes)", (unsigned)arena_size);
        return true;
    }
#endif

    s_tensor_arena = static_cast<uint8_t *>(
        heap_caps_aligned_alloc(16, arena_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)
    );
    if (s_tensor_arena != nullptr) {
        s_tensor_arena_size = arena_size;
        ESP_LOGI(kTag, "Tensor arena allocated in internal RAM (%u bytes)", (unsigned)arena_size);
        return true;
    }
    ...
    if (s_tensor_arena != nullptr) {
        s_tensor_arena_size = arena_size;
        ESP_LOGI(kTag, "Tensor arena allocated with generic heap (%u bytes)", (unsigned)arena_size);
        return true;
    }
...
esp_err_t quantize_or_copy_input(const uint8_t input_28x28[DIGIT_INFERENCE_INPUT_SIZE])
{
    if (s_input == nullptr) {
    ...
    ESP_LOGE(kTag, "Unsupported input tensor type: %d", static_cast<int>(s_input->type));
...
esp_err_t read_output_probabilities(float probs[DIGIT_INFERENCE_NUM_CLASSES])
{
    ...
    if (s_output->type == kTfLiteInt8) {
        ...
        const int n = std::min(
            DIGIT_INFERENCE_NUM_CLASSES,
            static_cast<int>(s_output->bytes)
        );
        for (int i = 0; i < n; ++i) {
            logits[i] = scale * (static_cast<float>(s_output->data.int8[i]) - static_cast<float>(zero_point));
        }
        ...
        ESP_LOGE(kTag, "Unsupported output tensor type: %d", static_cast<int>(s_output->type));
        return ESP_ERR_NOT_SUPPORTED;
    }
...
extern "C" esp_err_t digit_inference_init(void)
{
    ...
    if (s_model->version() != TFLITE_SCHEMA_VERSION) {
        ESP_LOGE(
            kTag,
            "Model schema mismatch. model=%d expected=%d",
            s_model->version(),
    ...
    const size_t arena_size = std::max<size_t>(16384, CONFIG_DIGIT_INFERENCE_TENSOR_ARENA_SIZE);
    if (!allocate_tensor_arena(arena_size)) {
        ESP_LOGE(kTag, "Failed to allocate tensor arena (%u bytes)", (unsigned)arena_size);
    ...
    ESP_LOGI(
        kTag,
        "TFLM initialized. arena_total=%u arena_used=%u input_type=%d output_type=%d",
...
)
{
    ...
    esp_err_t err = digit_preprocess_u8(input_gray, width, height, params, output_28x28);
    if (err != ESP_OK) {
        return err;
    }
